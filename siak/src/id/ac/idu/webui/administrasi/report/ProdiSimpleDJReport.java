 ...
 */
public class ProdiSimpleDJReport extends Window implements Serializable {
    ...
    public ProdiSimpleDJReport(Component parent) throws InterruptedException {
        super();
    ...
    public void doPrint() throws JRException, ColumnBuilderException, ClassNotFoundException, IOException {
        ...
        String filNr = Labels.getLabel("common.Prodi.ID");
        ...
        drb.setSubtitle("List of Prodis: " + ZksampleDateFormat.getDateTimeFormater().format(new Date()));
        ...
        ProdiService as = (ProdiService) SpringUtil.getBean("prodiService");
        List<Mprodi> resultList = as.getAllProdis();

        // Create Datasource and put it in Dynamic Jasper Format
        List data = new ArrayList(resultList.size());

        for (Mprodi obj : resultList) {
            Map<String, String> map = new HashMap<String, String>();
            map.put("ckdprogst", obj.getCkdprogst());
            map.put("cnmprogst", obj.getCnmprogst());
            map.put("csingkat", String.valueOf(obj.getCsingkat()));
            map.put("cstatus", String.valueOf(obj.getCstatus()));
            data.add(map);
        }
