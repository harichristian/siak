...
public class KurikulumMahasiswaMainCtrl extends GFCBaseCtrl implements Serializable {
	...
	public void onClick$button_KurikulumMahasiswaList_SearchCode(Event event) throws Exception {
		...
		if (!txtb_KurikulumMahasiswa_Code.getValue().isEmpty()) {
			...
			// ++ create the searchObject and init sorting ++//
			HibernateSearchObject<Mkurmhs> soKurikulumMahasiswa = new HibernateSearchObject<Mkurmhs>(Mkurmhs.class, getKurikulumMahasiswaListCtrl().getCountRows());
			soKurikulumMahasiswa.addFilter(new Filter(ConstantUtil.KURIKULUM, "%" + txtb_KurikulumMahasiswa_Code.getValue() + "%", Filter.OP_ILIKE));
			soKurikulumMahasiswa.addSort(ConstantUtil.KURIKULUM, false);
