...
public class DetailCtrl extends GFCBaseCtrl implements Serializable {
    ...
    public void searchMahasiswa(Filter... filters) {
		...
		soCuti.addFilter(new com.trg.search.Filter("mstatusmhs.ckdstatmhs", "2", com.trg.search.Filter.OP_EQUAL));
        if(filters != null) {
    ...
    public void onClick$buttonSearchTerm(Event event) {
        ...
        if (StringUtils.isNotEmpty(tbckdTerm.getValue()))
            filter1 = new Filter("kdTerm", "%" + tbckdTerm.getValue() + "%", Filter.OP_ILIKE);

        if (StringUtils.isNotEmpty(tbdeskripsi.getValue()))
            filter2 = new Filter("deskripsi", "%" + tbdeskripsi.getValue() + "%", Filter.OP_ILIKE);
