 ...
 */
public class HariSimpleDJReport extends Window implements Serializable {
    ...
    public HariSimpleDJReport(Component parent) throws InterruptedException {
        super();
    ...
    public void doPrint() throws JRException, ColumnBuilderException, ClassNotFoundException, IOException {
        ...
        String id = Labels.getLabel("common.Hari.ID");
        ...
        drb.addColumn(code, ConstantUtil.HARI_CODE, String.class.getName(), 50, columnStyleText, columnStyleTextBold);
        drb.addColumn(name, ConstantUtil.HARI_NAME, String.class.getName(), 50, columnStyleText, columnStyleTextBold);

        // Sets the Report Columns, header, Title, Groups, Etc Formats
        // DynamicJasper documentation
        drb.setTitle(this.zksample2title);
        drb.setSubtitle("Daftar Hari: " + ZksampleDateFormat.getDateTimeFormater().format(new Date()));
        ...
        HariService as = (HariService) SpringUtil.getBean("hariService");
        List<Mhari> resultList = as.getAllHaris();

        // Create Datasource and put it in Dynamic Jasper Format
        List data = new ArrayList(resultList.size());

        for (Mhari obj : resultList) {
            Map<String, String> map = new HashMap<String, String>();
            map.put(ConstantUtil.HARI_CODE, obj.getCkdhari());
            map.put(ConstantUtil.HARI_NAME, obj.getCnmhari());
            data.add(map);
        }
